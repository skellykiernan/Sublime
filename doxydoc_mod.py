# prvaite use, based on DoxyDoc implementation and modified
...
class DoxydocCommand(sublime_plugin.TextCommand):
    ...
    def template_function_snippet(self, regex_obj, template_args):
        print("template_function_snippet")
        snippet = ""
        index = 1
        snippet =  ("\n * {0}brief ${{{1}:[brief description]}}"
...
#        index += 2
        ...
        if args and args.lower() != "void":
            ...
            args = get_function_args(args)
            for _, name in args:
                snippet += "\n * {0}param[${{{2}:in,out}}] {1} ${{{3}:[description]}}\n * ".format(self.command_type, name, index, index + 1)
                index += 2
